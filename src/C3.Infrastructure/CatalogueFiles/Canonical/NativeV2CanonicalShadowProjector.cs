...
namespace C3.Infrastructure.CatalogueFiles.Canonical
{
    public sealed class NativeV2CanonicalShadowProjector
    {
        ...
        private static CatalogueState Map(NativeCatalogue source)
        {
            ...
            {
                var sideA = Side(value.SideA, recordings);
                var sideB = Side(value.SideB, recordings);
                tapes.Add(new CatalogueTapeState(
                    Key(CatalogueEntityKind.Tape, value.Id.ToString()),
                    Key(CatalogueEntityKind.CassetteModel, value.CassetteModelId.ToString()),
                    value.Year,
                    value.LengthMinutes,
                    value.Region,
                    value.Number,
            ...
            var metadata = new CatalogueMetadataState(
            ...
            ICollection<CatalogueRecordingState> recordings)
        {
            ...
            if (source.Recording.HasValue)
            {
                ...
                recordingId = Key(CatalogueEntityKind.Recording, value.Id.ToString());
        ...
        private static IEnumerable<CatalogueEntityCount> Counts(CatalogueState state)
        {
            return new[]
            {
                new CatalogueEntityCount(CatalogueEntityKind.Brand, state.Brands.Count),
                new CatalogueEntityCount(CatalogueEntityKind.CassetteModel, state.CassetteModels.Count),
                new CatalogueEntityCount(CatalogueEntityKind.DeckModel, state.DeckModels.Count),
                new CatalogueEntityCount(CatalogueEntityKind.DeckUnit, state.DeckUnits.Count),
                new CatalogueEntityCount(CatalogueEntityKind.Tape, state.Tapes.Count),
                new CatalogueEntityCount(CatalogueEntityKind.Recording, state.Recordings.Count)
            };
        }

        private static CatalogueEntityKey Key(CatalogueEntityKind kind, string id)
        {

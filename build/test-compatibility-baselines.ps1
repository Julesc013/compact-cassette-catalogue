...
$nativeExportOutput = Join-Path $workRoot 'native-export-v1.1.xml'
& $testExecutable '--write-native-export-v1.1' $nativeExportOutput
if ($LASTEXITCODE -ne 0) {
    throw 'The native-to-v1.1 export probe failed.'
}
...
foreach ($release in @($corpus.releases | Where-Object { $_.support -ceq 'supported' })) {
    foreach ($artifact in $release.artifacts) {
        ...
        foreach ($input in @(
                [PSCustomObject]@{ id = 'current-writer'; path = $currentWriterOutput },
                [PSCustomObject]@{ id = 'native-export'; path = $nativeExportOutput })) {
            $roundTrip = Join-Path $workRoot (
                ([IO.Path]::GetFileNameWithoutExtension([string]$artifact.name)) +
                '-' + $input.id + '-round-trip.xml')
            $probeArguments = @(
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "Historical reader/writer probe failed: $($artifact.name), $($input.id)"
            }

            & $testExecutable '--validate-v1.1' $roundTrip
            if ($LASTEXITCODE -ne 0) {
                throw "Current reader rejected historical writer output: $($artifact.name), $($input.id)"
            }

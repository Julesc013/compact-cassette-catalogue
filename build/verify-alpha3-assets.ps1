...
if (($actualAssetNames -join "`n") -cne (($expectedAssetNames | Sort-Object) -join "`n")) {
    throw "$($manifest.releaseLabel) distribution is not the exact six-ZIP/checksum set: $($actualAssetNames -join ', ')"
}
...
if (($actualEvidenceNames -join "`n") -cne (($expectedEvidenceNames | Sort-Object) -join "`n")) { throw "$($manifest.releaseLabel) release-asset evidence is not the exact record/checksum pair." }
...
if ([int]$record.schemaVersion -ne 1 -or [string]$record.classification -cne $expectedClassification -or
        ...
        [string]$record.releaseTag -cne [string]$manifest.releaseTag -or [string]$record.releaseChannel -cne [string]$manifest.releaseChannel -or
        [string]$record.publicationStatus -cne [string]$manifest.publicationStatus -or [string]$record.sourceCommit -notmatch '^[0-9a-f]{40}$' -or
        [string]$record.toolchainLockSha256 -notmatch '^[0-9a-f]{64}$' -or @($record.assets).Count -ne 6) {
    throw "$($manifest.releaseLabel) release-asset record has invalid classification/release/source/lock identity."
}
$checksumLines = @(Get-Content -LiteralPath (Join-Path $DistributionDirectory 'SHA256SUMS.txt'))
if ($checksumLines.Count -ne 6) { throw "$($manifest.releaseLabel) SHA256SUMS.txt must contain exactly six lines." }
...
for ($index = 0; $index -lt $expectedOrder.Count; $index++) {
    ...
    if ($checksumLines[$index] -cne "$hash  $assetName" -or $asset.Count -ne 1 -or [string]$asset[0].sha256 -cne $hash -or
        ...
        throw "$($manifest.releaseLabel) release asset/checksum/evidence mismatch: $assetName"
    }
    ...
    if ((Get-FileHash -LiteralPath (Join-Path $sourceDirectory $assetName) -Algorithm SHA256).Hash.ToLowerInvariant() -cne $hash) {
        throw "$($manifest.releaseLabel) assembled asset differs from its independently verified package: $assetName"
    }
    ...
    if ([string]$asset[0].entryManifestName -cne "$assetName.entries.json" -or
        ...
        throw "$($manifest.releaseLabel) asset is not bound to its authenticated entry manifest: $assetName"
    }
}
Write-Host "Verified the retained $($manifest.releaseLabel) distribution as exactly six independently verified ZIPs, one checksum manifest, one release identity, one source, and one lock."

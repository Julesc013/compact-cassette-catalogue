...
if ((& git -C $repositoryRoot rev-parse 'v1.3.0a4^{commit}').Trim() -cne $head) { throw 'v1.3.0a4 does not point to the current evidence commit.' }
$tagText = (& git -C $repositoryRoot cat-file tag refs/tags/v1.3.0a4) -join "`n"
Assert-C3Alpha4TagMessage -Text $tagText
$record = Get-Content -LiteralPath $RecordPath -Raw | ConvertFrom-Json
if ([string]$record.sourceCommit -cne (& git -C $repositoryRoot rev-parse 'HEAD^').Trim() -or @($record.assets).Count -ne 6) { throw 'Tagged Alpha 4 record does not bind the direct package-source parent and six assets.' }
...
if ($VerifyRemote) {
    ...
    if ($remoteObject -cne (& git -C $repositoryRoot rev-parse refs/tags/v1.3.0a4).Trim() -or $remoteTarget -cne $head) { throw 'Remote Alpha 4 tag differs from the local annotated object/target.' }
}
Write-Host "Verified annotated v1.3.0a4 and its unchanged six retained test assets at $head."
